...
(
	# If nasux bootstrap second stage has never been run, like in case
	# bootstrap was extracted to rootfs from a shell instead of by the
	# by the NasUX app, which normally runs the second stage, then run it.
	# This is currently an issue of pacman bootstraps, which are not
	# supported by the NasUX app and both extraction and second stage
	# are run from a shell. Once support has been added, this script
	# will be removed.
	# NasUX app wipes the prefix directory if second stage fails,
	...
	if [ ! -L "@TERMUX_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR@/@TERMUX_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@.lock" ]; then
		echo "Starting fallback run of nasux bootstrap second stage"
	...
	rm -f "@TERMUX__PREFIX__PROFILE_D_DIR@/01-nasux-bootstrap-second-stage-fallback.sh" || exit $?
