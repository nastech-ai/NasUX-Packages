...
nasux_setup_zig() {
	...
	*) termux_error_exit "Please add ${TERMUX_ZIG_VERSION} archive checksum to nasux_setup_zig and update patches in packages/zig" ;;
	esac
	...
	if [[ ! -x "${ZIG_FOLDER}/zig" ]]; then
		...
		if [[ -n "$(find "${TERMUX_SCRIPTDIR}/packages/zig/${TERMUX_ZIG_VERSION}" -name 'zig-*.patch')" ]]; then
			echo "nasux_setup_zig: Applying patches from packages/zig/${TERMUX_ZIG_VERSION}"
			local p
		...
		if [[ -n "$(find "${TERMUX_SCRIPTDIR}/packages/zig/${TERMUX_ZIG_VERSION}" -name '*.diff')" ]]; then
			echo "nasux_setup_zig: Applying diff from packages/zig/${TERMUX_ZIG_VERSION}"
			local p
	...
	if [[ -z "$(command -v zig)" ]]; then
		termux_error_exit "nasux_setup_zig: No zig executable found!"
	fi
}

# Backward-compatibility alias for packages that call termux_setup_zig
termux_setup_zig() { nasux_setup_zig "$@"; }
