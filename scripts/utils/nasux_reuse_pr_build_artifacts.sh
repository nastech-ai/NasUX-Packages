...
ci_artifact_url() {
	curl --silent \
		-H "Authorization: token ${GITHUB_TOKEN}" \
		-H "Accept: application/vnd.github.v3+json" \
		"https://api.github.com/repos/nastech-ai/NasUX-Packages/actions/runs/${1}/artifacts" \
...
download_ci_artifacts() {
	...
	CI_ARTIFACT_ZIP="$HOME/.nasux-build/_cache/artifact-${WORKFLOW_ID}.zip"
	...
	mkdir -p "$HOME/.nasux-build/_cache"
...
[[ "${GITHUB_EVENT_NAME:-}" == "push" ]] && {
	...
	RELATED_PRS_QUERY="
	query {
		repository(owner: \"nastech-ai\", name: \"NasUX-Packages\") {
		$(
	...
	if (( ${#PR_BASE_TO_HEAD_CHANGED_BUILDSYSTEM_FILES[*]} + ${#PR_BASE_TO_HEAD_CHANGED_DEPS[*]} + PR_CI_REUSE )); then
		...
		if [[ -n "${WORKFLOW_ID}" ]]; then
			echo "We can safely reuse CI artifacts from https://github.com/nastech-ai/NasUX-Packages/actions/runs/${WORKFLOW_ID}"
