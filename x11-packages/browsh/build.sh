NASUX_PKG_HOMEPAGE=http://www.brow.sh/
NASUX_PKG_DESCRIPTION="A fully-modern text-based browser, rendering to TTY and browsers"
NASUX_PKG_LICENSE="LGPL-2.1"
NASUX_PKG_MAINTAINER="@nastech-ai"
NASUX_PKG_VERSION=1.8.3
NASUX_PKG_SRCURL=(
	"https://github.com/browsh-org/browsh/archive/refs/tags/v$NASUX_PKG_VERSION.tar.gz"
	"https://github.com/browsh-org/browsh/releases/download/v$NASUX_PKG_VERSION/browsh-$NASUX_PKG_VERSION.xpi"
)
NASUX_PKG_SHA256=(
...
termux_extract_src_archive() {
	...
	tarball="$TERMUX_PKG_CACHEDIR/$(basename "${NASUX_PKG_SRCURL[0]}")"
	mkdir -p "$TERMUX_PKG_SRCDIR"
	tar -xf "$tarball" -C "$TERMUX_PKG_SRCDIR" --strip-components=1

	extension="$TERMUX_PKG_CACHEDIR/$(basename "${NASUX_PKG_SRCURL[1]}")"
