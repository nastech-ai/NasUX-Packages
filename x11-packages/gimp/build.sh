NASUX_PKG_HOMEPAGE=https://www.gimp.org/
NASUX_PKG_DESCRIPTION="GNU Image Manipulation Program"
NASUX_PKG_LICENSE="GPL-3.0, LGPL-3.0"
NASUX_PKG_MAINTAINER="@nastech-ai"
NASUX_PKG_VERSION="3.2.0"
TERMUX_PKG_REVISION=1
NASUX_PKG_SRCURL=git+https://gitlab.gnome.org/GNOME/gimp
TERMUX_PKG_GIT_BRANCH="GIMP_${NASUX_PKG_VERSION//./_}"
TERMUX_PKG_AUTO_UPDATE=true
NASUX_PKG_DEPENDS="aalib, appstream, appstream-glib, atk, babl, fontconfig, freetype, gdk-pixbuf, gegl, gexiv2, ghostscript, gimp-data, glib, glib-networking, gtk3, harfbuzz, hicolor-icon-theme, imath, json-glib, libandroid-execinfo, libandroid-shmem, libbz2, libc++, libcairo, libheif, libjpeg-turbo, libjxl, libmypaint, libpng, librsvg, libtiff, libwebp, libxcursor, libxmu, libxpm, littlecms, mypaint-brushes, openexr, openjpeg, pango, poppler, poppler-data, pygobject, zlib"
...
-Dlua=false
-Dbug-report-url=https://github.com/nastech-ai/NasUX-Packages/issues/new?template=01-bug-report.yml
"
...
termux_step_pre_configure() {
	...
	if [ "$TERMUX_ON_DEVICE_BUILD" = "false" ]; then
		...
		termux_download https://download.gimp.org/gimp/v${NASUX_PKG_VERSION%.*}/linux/GIMP-${NASUX_PKG_VERSION}-x86_64.AppImage "$TERMUX_PKG_CACHEDIR/gimp.appimage" e7794ebd6ed90fe32f6dede3917216aaceee833bdc794f2a0fe925e1d96cc899
		...
		cat > "$TERMUX_PKG_TMPDIR/bin/gtk-encode-symbolic-svg" <<-HERE
			...
			exec $(command -v nasux-proot-run) env LD_PRELOAD= LD_LIBRARY_PATH= $TERMUX_PREFIX/bin/gtk-encode-symbolic-svg "\$@"
		HERE
		...
		HERE
		# meson cross-file already has pkg-config defined so nasux's pkg-config wrapper will be used for cross-compilation.
