...
/**
 * Creates a NasUX:GUI backend, and connection to the NasUX:GUI plugin.
 * A NasUX:GUI backend has no outputs or inputs by default.
 */
struct wlr_backend *wlr_tgui_backend_create(struct wl_event_loop *loop);
/**
 * Create a new NasUX:GUI output.
 *
 * Will use NasUX:GUI plugin to create Activity and SurfaceView, the buffers presented
